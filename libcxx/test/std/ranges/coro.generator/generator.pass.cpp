...
static_assert(std::ranges::__begin::__member_begin<std::add_lvalue_reference_t<decltype(fib())>>);
static_assert(std::ranges::input_range<std::add_lvalue_reference_t<decltype(fib())>>);
...
bool test() {
  {
    const std::vector<int> expected_fib_vec = {0, 1, 1, 2, 3};
    {
